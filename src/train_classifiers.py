"""
Train one classifier head.

This records how the published heads were fitted, with the hyperparameters that
were used. Use this to see the procedure, or to fit classifiers on your own data. To check our
...
Features are read from <fpts_dir>/<fp_type>_fpts/{train,test}_set_fingerprint.npz and used unscaled, 
matching the finetuning pipeline. Mordred is the exception, its descriptors are standardised, and 
src/hdac/models.py records how each saved head expects that to be applied.
"""
import argparse
import os
import sys
